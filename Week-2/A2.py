...
class FourNodeElement:
    '''
    This class is used to represent Two-node elements in the circuit\n
    It has following attributes\n
        1. Name\n
        2. From dependent node\n
        3. To dependent node\n
        4. From node\n
        5. To node\n
        6. Value\n
    ...
    def __init__(self, componentattrs:list):
        ...
        self.compName = self.componentattrs[0]
        self.compValue = float(self.componentattrs[-1])
        self.fromDNode = self.componentattrs[1]
        self.toDNode = self.componentattrs[2]
        self.fromNode = self.componentattrs[3]
        self.toNode = self.componentattrs[4]
        if self.fromDNode.lower() == 'gnd':
            self.fromDNode = 'GND'
        if self.toDNode.lower() == 'gnd':
            self.toDNode = 'GND'
        if self.fromNode.lower() == 'gnd':
            self.fromNode = 'GND'
        if self.toNode.lower() == 'gnd':
...
class CurrentSource(TwoNodeElement):
    ...
    def updateACMat(self, M, B):
        '''Updates the M & B matrices according to the Current source'''
        fromNodeInd = Nodes.index(self.fromNode)
        toNodeInd = Nodes.index(self.toNode)
        try:
            assert self.fromNode != 'GND'
...
class VCVS(FourNodeElement):
    '''
    This class is used to represent Voltage Controlled Voltage Source in the circuit\n
    This class inherits from FourNodeElement class
    '''
    def __init__(self, componentattrs:list):
        self.compType = 'VCVS'
        FourNodeElement.__init__(self, componentattrs)
        if self.fromNode not in Nodes:
            Nodes.append(self.fromNode)
        if self.toNode not in Nodes:
            Nodes.append(self.toNode)
    
    def updateDCMat(self):
        '''Updates the M & B matrices according to VCVS'''
